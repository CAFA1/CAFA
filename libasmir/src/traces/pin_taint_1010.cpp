...
VOID  CheckConditionalJMP(ADDRINT iaddr,THREADID tid,bool taken,ADDRINT target)
{
  ...
  if (t.size()>iTNTChksmDegree)
    SysLog << "[HIGH-TNT_JMP] " << "PC " << hex <<iaddr << " count "<<t.size()<<endl;
  return;
}
...
VOID InstructionProp(INS ins, VOID *v)
{
  ...
  //调试状态下，打印每一条指令及对应所在函数，addbyrichard:&&!CompactLog
  //if (KnobDebug&&!CompactLog)
  //{
  //  RTN rtn = INS_Rtn(ins);
  //  string rtnname("notfound");
  //  if (RTN_Valid(rtn))
  //  {
  //    rtnname = RTN_Name(rtn);
  //  }
  //  TraceFile << "[DBG] " << rtnname << " propagation for instr " 
  //    << INS_Disassemble(ins) << " PC " << hex << iaddr << endl;
  //}
  ...
  gr_read = gr_write = xt_read = xt_write = eflag_write=0;
    ...
    //count_reg_read 0x2 count_reg_write 0x2 memRead 0 memWrite 0
    else if(count_reg_read==2 && count_reg_write==2 && memRead==0 && memWrite==0)
    {
      if(REG_is_dift_eflag(static_cast<REG>(index_reg_write)))
      {
        TraceFile<<" R2->R2: "<<INS_Disassemble(ins)<<" iaddr "<<hex<<iaddr<<endl;

        IFCOND(ins);   
        INS_InsertThenCall(ins,IPOINT_BEFORE, 
          (AFUNPTR) liuR2_R2,IARG_FAST_ANALYSIS_CALL,
          IARG_UINT32, iaddr,
          IARG_UINT32, index_reg_read,
          IARG_UINT32, index_reg_write,
          IARG_UINT32, index_reg_read1,
    ...
    else if(count_reg_read==0 && count_reg_write==1 && memRead==0 && memWrite==0)
    {
      if(REG_is_dift_eflag(static_cast<REG>(index_reg_write)))
      {
        TraceFile<<" 0->R: "<<INS_Disassemble(ins)<<" iaddr "<<hex<<iaddr<<endl;

        IFCOND(ins);   
        INS_InsertThenCall(ins,IPOINT_BEFORE, 
          (AFUNPTR) liu0_R,IARG_FAST_ANALYSIS_CALL,
          IARG_UINT32, iaddr,
          //IARG_UINT32, index_reg_read,
          IARG_UINT32, index_reg_write,
    ...
    else if(count_reg_read==1 && count_reg_write==1 && memRead==0 && memWrite==0)
    {
      if(REG_is_dift_eflag(static_cast<REG>(index_reg_write)))
      {
        TraceFile<<" R->R: "<<INS_Disassemble(ins)<<" iaddr "<<hex<<iaddr<<endl;
    ...
    else if(count_reg_read==1 && count_reg_write==2 && memRead==0 && memWrite==0)
    {
      if(REG_is_dift_eflag(static_cast<REG>(index_reg_write)))
      {
        TraceFile<<" R->R2: "<<INS_Disassemble(ins)<<" iaddr "<<hex<<iaddr<<endl;

        IFCOND(ins);   
        INS_InsertThenCall(ins,IPOINT_BEFORE, 
          (AFUNPTR) liuR_R2,IARG_FAST_ANALYSIS_CALL,
          IARG_UINT32, iaddr,
          IARG_UINT32, index_reg_read,
          IARG_UINT32, index_reg_write,
          IARG_UINT32, index_reg_write1,
    ...
    if(count_reg_read==2 && count_reg_write==1 && memRead==0 && memWrite==0)
    {
      if(REG_is_dift_eflag(static_cast<REG>(index_reg_write)))
      {
        TraceFile<<" R2->R: "<<INS_Disassemble(ins)<<" iaddr "<<hex<<iaddr<<endl;
        IFCOND(ins);   
        INS_InsertThenCall(ins,IPOINT_BEFORE, 
    ...
    else if(count_reg_read==1 && count_reg_write==0 && memRead==0 && memWrite==1)
    {
      TraceFile<<" R->M: "<<INS_Disassemble(ins)<<" iaddr "<<hex<<iaddr<<endl;
      IFCOND(ins);
      INS_InsertThenCall(ins,IPOINT_BEFORE, 
      (AFUNPTR) liuR_M,IARG_FAST_ANALYSIS_CALL,
      IARG_UINT32, iaddr,
    ...
    else if(count_reg_read==0 && count_reg_write==1 && memRead==1 && memWrite==0)
    {
      if(REG_is_dift_eflag(static_cast<REG>(index_reg_write)))
      {
        TraceFile<<" M->R: "<<INS_Disassemble(ins)<<endl;
        IFCOND(ins);
        INS_InsertThenCall(ins,IPOINT_BEFORE, 
        (AFUNPTR) liuM_R,IARG_FAST_ANALYSIS_CALL,
        IARG_UINT32, iaddr,
        IARG_PTR, new string(INS_Disassemble(ins)),
        IARG_UINT32, index_reg_write,
        IARG_MEMORYREAD_EA , 
    ...
    {
      TraceFile<<" Read2: "<<INS_Disassemble(ins)<<endl;
    }
    else
    {
      TraceFile<<" other: "<<INS_Disassemble(ins)<<" count_reg_read "<<count_reg_read<<" count_reg_write "<<count_reg_write<<" memRead "<<memRead<<" memWrite "<<memWrite<<endl;
       for(ii=0; ii < valcount; ii++) 
      {
        //TraceFile<< "here instructions: "<<INS_Disassemble(ins)<<endl;
        ...
        if((opndvals[ii].taint&RD)!=0 && opndvals[ii].type.type == REGISTER)
        {
            ...
            TraceFile<< "read: "<<REG_StringShort((LEVEL_BASE::REG)opndvals[ii].reg)<<endl;
          
        }
        if((opndvals[ii].taint&WR)!=0 && opndvals[ii].type.type == REGISTER)
        {
            TraceFile<< "write: "<<REG_StringShort((LEVEL_BASE::REG)opndvals[ii].reg)<<endl; 
        }
