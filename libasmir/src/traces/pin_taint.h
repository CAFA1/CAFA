...
extern ADDRINT WriteBlock(THREADID threadid,ADDRINT addr,bool taken);
extern void SetRegisterTaint(bool is_tainted, REG r, THREADID id, ADDRINT iaddr);
extern char buf_file_addrs[50];
extern list <branch_st> g_bbls;
extern VOID InstructionProp(INS ins, VOID *v);
extern unsigned int iTNTChksmDegree;
typedef bool (*InstrumentFunction)(INS ins);
extern bool TAINT_Instrumentation_On ;
extern bool crc32_Instrumentation_On ;
...
typedef struct TNT_mem
{
  ...
  ADDRINT addr_end;
  set<int> instr;
    unsigned int iMaxTntDgr;
}TNT_mem;

typedef struct {
  ADDRINT syscall_stack;
  // ADDRINT taintloc;
  ADDRINT size;
  bool reg_taint[REG_XMM_LAST+1];
  //by richard
  //set<int> reg_taintsrc_table[REG_XMM_LAST+1]; //寄存器污点来源映射表
  set<int>* ptaintsrc_table;
  int test[REG_XMM_LAST+1]; //寄存器污点来源映射表
  bool taint_prop;
  bool in_filetaint;
  char reserved2;
  //funcsum* curr_func;
  ADDRINT function_info;
  ADDRINT last_taint_addr;
  ADDRINT last_taint_size;
  ADDRINT last_taint_ins;
  ADDRINT last_utaint_addr;
  ADDRINT last_utaint_size;
  ADDRINT last_utaint_ins;
  std::ofstream trace_file;                   
} per_thread;
#define MAX_NUM_CPUS 12

//liu 911 global
 typedef struct{
  ADDRINT memaddr; //内存地址
  set<int> srcOffsets; //污点来源偏移
}TaintSourceEntry; 
extern VOID Setmem_taint(ADDRINT addr, UINT32 size, bool is_tainted, ADDRINT inst);
extern void SetRegisterTaintSrc(set<int> taintsrc, REG r, THREADID id, ADDRINT iaddr);
extern per_thread thread_info1[MAX_NUM_CPUS];
#define TAINT_TABLE_SIZE 134217723 // 1GB
extern char mem_taint[TAINT_TABLE_SIZE+1];
#define getByteMask(addr) (1 << ((unsigned int)addr % 8))
#define getTableIndex(addr) (((unsigned int)addr >> 3) % TAINT_TABLE_SIZE)
extern PIN_LOCK lock;
extern PIN_LOCK lock1;
extern bool TAINT_Analysis_On ;
extern set<int> perthread_taintsrctable[MAX_NUM_CPUS][REG_XMM_LAST+1]; //寄存器污点来源映射表,perthread存放指针
extern VOID dump_taint_src();
/* Wirtten by richhard                           
   污点源内存管理结构*/
/************************************************************************/
  ...
  typedef std::pair<string, uint64_t> resource_t;
   ...
   public:
	...
	void trackOffset(uint32_t offset, uint32_t length);
     ...
     void OpenHelper(uint32_t fd,char* s);   
