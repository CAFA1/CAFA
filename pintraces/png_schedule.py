...
def make_samples():
    ...
    f1.write("a"*20+crcfilestr)
    f1.close()
def run_cmd(offsets1,offsets2,coverage,elfpath,filepath,check):
     
    filename = filepath.split("/")[-1]
    pin_cmd = "/home/bap/workspace/bap-0.7/pin/pin -t /home/bap/workspace/bap-0.7/pintraces/obj-ia32/gentrace.so -taint-offsets "+offsets1+" -taint-offsets "+offsets2+"  -o 1-1 -log-limit 10000 -ins-limit 1000000 -time-limit 35  -c "+coverage+" -check "+check+" -taint-files "+filename+" --  "+elfpath+" identify "+filepath
...
def compare_run(offsets1,offsets2,coverage,elfpath,check):
    run_cmd(offsets1,offsets2,coverage,elfpath,good,check)
    jz_good_list = get_jz()
    bbl_good = get_bbl()
    #cleandir()
    run_cmd(offsets1,offsets2,coverage,elfpath,bad,check)
    bbl_bad = get_bbl()
    result_jz=set()
    for jz in jz_good_list:
        if(bbl_good.has_key(jz) and bbl_bad.has_key(jz)):
...
def main(argv=sys.argv):
    ...
    elfpath = argv[4]
    check = argv[5]
    result_jz=compare_run(offsets1,offsets2,coverage,elfpath,check)
    print "result"
