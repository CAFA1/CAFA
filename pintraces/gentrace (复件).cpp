...
//liu 911
#define SysLog ((thread_info[0].trace_file))
// TraceFile points to the thread-local trace file
#define TraceFile ((thread_info[(PIN_ThreadId()<0)? 0 :PIN_ThreadId()].trace_file))
//liu 911
#ifdef _WIN32
...
//liu 911 instruction instrument class
...
VOID PIN_FAST_ANALYSIS_CALL printInstruction(ADDRINT iaddr, ADDRINT target, THREADID id)
{ 
  if (PrintIns)
    thread_info[id].trace_file << "PC:" << iaddr << endl; //" Calling " << target << "\n";
}
...
{
  return (thread_info[tid].taint_prop && TAINT_Analysis_On);
}
...
__GetRegisterTaint(REG r,  THREADID id) {
  return (thread_info[id].reg_taint[r] != false);
}
static bool REG_is_dift(REG r)
{
    //多媒体扩展寄存器
...
static bool REG_is_dift_eflag(REG r)
{
  ...
  if (REG_is_gr(r))
  {
    //Updated by richard in 2012-11-6: add the effect to EFLAGS
    //不处理EIP,EFLAGS和ESP
    if (r == REG_EIP || r == REG_ESP)
      return false;
    return true;
  }
  //处理到这一定是gr8和gr16（8位的和）
...
__SetRegisterTaint(bool isTainted, REG r, THREADID id) {
    thread_info[id].reg_taint[r] = isTainted;
}
//by richard ,获取寄存器污染源集合
 set<int> 
__GetRegisterTaintSrc(REG r,THREADID id) {
    return (thread_info[id].ptaintsrc_table[r]);
}
//by richard ,设置寄存器污染源集合
 void 
__SetRegisterTaintSrc(set<int> taintsrc, REG r, THREADID id) {
    //thread_info[id].reg_taintsrc_table[r]=taintsrc;
    thread_info[id].ptaintsrc_table[r]=taintsrc;
}
                                           ...
                                           THREADID id)
{
   ...
   if(GetRegisterTaint(REG_ECX,id,iaddr))
   {
       ...
       if (!t.empty())
       {
           TraceFile<<" REP ECX taint Source: "<<PrintSet(t)<<" Content: "<<printMemdata((char*)memsrc,memsrcsz,false)<<endl;
       }
